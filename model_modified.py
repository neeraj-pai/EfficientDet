...
class ClassNet(models.Model):
    ...
    def call(self, inputs, **kwargs):
        ...
        for i in range(self.depth):
            ...
            feature = self.bns[i][self.level](feature)
            feature = self.relu(feature)
        last_conv_out = self.head(feature)#output of last convolutional layer
        
        classifier_input = keras.Input(shape=last_conv_out.shape[1:])
        x = classifier_input
        x = self.reshape(x)
        x = self.activation(x)
        classifier_model = keras.Model(classifier_input, x)

        #outputs = self.reshape(last_conv_out)
        #outputs = self.activation(outputs)
        self.level += 1
        return last_conv_out,classifier_model


def efficientdet_mod(phi, num_classes=20, num_anchors=9, weighted_bifpn=False, freeze_bn=False,
    ...
    convolution_layer_model_out = []
    classification_layer_model_out = []
    for i, feature in enumerate(fpn_features):
        conv_out,classification_out_model = class_net([feature, i])
        conv_layer_out_model = models.Model(inputs=[image_input], outputs=conv_out)
        convolution_layer_model_out.append(conv_layer_out_model)
        classification_layer_model_out.append(classification_out_model)    
    
    return convolution_layer_model_out,classification_layer_model_out
